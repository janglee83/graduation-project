...
class HarmonySearch(BaseModel):
    ...
    def initialize_harmony_memory(self, lower_upper_tensor: Tensor, num_row: int, num_col: int, num_item: int, listTaskAssign: list[TaskAssignRequest]) -> Tensor:
        truncated_normal = TruncatedNormalService()

        task_size_tensor = torch.zeros(num_row, num_col, dtype=torch.long)

        for task_assign in listTaskAssign:
            row = (task_assign.task_id - 1) % num_row
            col = task_assign.metric_id - 1
            task_size_tensor[row, col] = len(task_assign.resource_ids)
        ...
        while True:
            ...
            for row in range(num_row):
                for col in range(num_col):
                    size_len = task_size_tensor[row, col].item()

                    # Define bounds
                    lower_bound = lower_bounds[row, col].item()
                    upper_bound = upper_bounds[row, col].item()
                    ...
                    task_weight = truncated_normal.generate_truncated_normal_with_sum(
                        size=size_len)
                    num_elements_to_fill = min(size_len, num_item)

                    harmony[row, col,
                            :num_elements_to_fill] = task_weight[:num_elements_to_fill]
