...
class AntColony(BaseModel):
    ...
    def __init__(self, number_ants: int, number_edge: int, relationship_kpi_matrix: Any, pheromone_matrix: Tensor, duration_matrix: Any, best_ant_path: List = [], best_ant_path_length: float = 0.0, alpha: float = 0.1, beta: float = 1, default_pheromone_value: float = 1.0, pl: float = 0.8, pg: float = 0.95) -> None:
    ...
    def get_best_next_point(self, list_reachable_point: list, harmony_search: HarmonySearch, num_row: int, listTaskLinkage: list[TaskLinkageRequest], antWeight: torch.Tensor, num_task: int):
        ...
        probabilities = ((1 / torch.sum(harmony_search.harmony_memory, dim=3)) ** self.beta) * (
            self.pheromone_matrix ** self.alpha) * ((1 / torch.sum(self.duration_matrix, dim=2)) ** (self.beta + 2))
        ...
        if not (len(list_not_reachable_point) == 1 and list_not_reachable_point[-1] == FINISH_POINT_NAME):
            for point in list_not_reachable_point:
                if point != START_POINT_NAME:
                    ...
                    if kpi_id is not None:
                        ...
                        prob_tensor[:, row_index, col_index] = torch.tensor(0.0)

            max_position = torch.argmax(prob_tensor)

            # Calculate indices manually
            selected_kpi = max_position % prob_tensor.shape[2]
            selected_task_in_kpi = (
                max_position // prob_tensor.shape[2]) % prob_tensor.shape[1]
            selected_hms = max_position // (prob_tensor.shape[1] * prob_tensor.shape[2])

            # Convert to Python integers
            selected_hms = selected_hms.item()
            selected_task_in_kpi = selected_task_in_kpi.item()
            selected_kpi = selected_kpi.item()
            ...
            antWeight[selected_task_in_kpi, selected_kpi, :] = harmony_search.harmony_memory[selected_hms, selected_task_in_kpi, selected_kpi, :]

            return point, torch.tensor([selected_hms, selected_task_in_kpi, selected_kpi]), torch.sum(self.duration_matrix[selected_task_in_kpi, selected_kpi, :])
