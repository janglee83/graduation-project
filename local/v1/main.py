...
with open('./human_60/data.json') as f:
    d = json.load(f)
...
for index, item in data_frame_employee.iterrows():
    employee_ins = EmployeeRequest(id=str(
...
with open('./human_5/environment_effect.json') as file:
    data = json.load(file)
    listEnvironmentsEffectScore = create_instances_from_json(
        listItems=data, Model=EnvironmentEffect)

with open('./human_5/equipment_effect.json') as file:
    data = json.load(file)
    listEquipmentEffectScore = create_instances_from_json(
        listItems=data, Model=EquipmentEffect)

with open('./human_5/human_effect.json') as file:
    data = json.load(file)
    listHumanEffectScore = create_instances_from_json(
        listItems=data, Model=HumanEffect)

with open('./human_5/product_effect.json') as file:
    data = json.load(file)
    listProductEffectScore = create_instances_from_json(
        listItems=data, Model=ProductEffect)

timer.end()
timer.elapsed_time("Finish read data in: ")
...
timer.end()
timer.elapsed_time("Finish build request data in: ")
...
timer.end()
timer.elapsed_time("Finish run algorithms in: ")
